...
func testRegexpGenerator2(t *testing.T, pattern string) {
	i := 0

	for i < 10 {
		reg := NewBuilder([]byte(pattern))
		reg.Build()
		result := reg.RenderResult()
...
func TestRegexpCompile_SetPatterns_Success(t *testing.T) {
	patternSet := []string{
		"[a-z]{5,6}",
		"[a-z]{5,6} test [a-zA-Z]{9}",
		`\+7 \([489][0-9]{2}\) [0-9]{3} [0-9]{2} [0-9]{2}`,
	...
	for _, pattern := range patternSet {
		testRegexpGenerator2(t, pattern)
	}
}